...
### Windows
Дважды кликните по **`install-shortcut.bat`** — на рабочем столе появится ярлык **«PlanPlace Keystatic»**. Дальше просто кликайте по ярлыку: откроется небольшое окно консоли (сервер) и браузер с админкой. Чтобы остановить — закройте это окно.

**Тихий режим (без окна консоли):** вместо ярлыка можно использовать **`start-hidden.vbs`** — сделайте ярлык на рабочем столе на него (ПКМ по файлу → «Отправить» → «Рабочий стол (создать ярлык)»). Тогда при запуске ничего не мелькает, открывается только браузер. Минус: чтобы остановить сервер, нужно завершить процесс **node.exe** в Диспетчере задач.
