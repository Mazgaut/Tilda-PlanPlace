...
## Обязательный навык для работы со статьями

Все ИИ-модели и агенты, которые создают, дополняют, проверяют или перерабатывают
статьи этого справочного центра, должны использовать проектный навык
[`planplace-user-docs-writer`](.agents/skills/planplace-user-docs-writer/SKILL.md).
Если среда не подключила навык автоматически, перед началом работы нужно открыть
его `SKILL.md` вручную и выполнить описанный в нём порядок проверки.

Навык содержит общие границы работы и ссылки на обязательные материалы:

- [правила оформления статей](.agents/skills/planplace-user-docs-writer/references/article-format.md) — структура для новичка, терминология, MDX, дата обновления, SEO, GEO и микроразметка;
- [проверка фактов по PlanPlace](.agents/skills/planplace-user-docs-writer/references/research-and-verification.md) — сверка с основной веткой core_2, переводами и доступным локальным интерфейсом.

Прямые указания пользователя для конкретной задачи имеют приоритет над общими
рекомендациями навыка. Код PlanPlace используется как источник истины и не
изменяется при подготовке справочных материалов.
