...
> **ВАЖНО.** PlanPlace не делает раскрой и не знает ваш КИМ (коэффициент использования материала). Для расчёта **по штукам** встроенный [коэффициент отхода](/Tilda-PlanPlace/plitnye-materialy/) не применяется — запас здесь уже даёт округление вверх до целого листа. Если этого мало, заложите отход в цену листа, уменьшите размер листа под реальный КИМ или добавьте [наценку](/Tilda-PlanPlace/pravila-rascheta-cen-nacenki/) на материал.
...
> Оптимизация раскроя не выполняется, зато для расчёта по площади работает встроенный **[коэффициент отхода, %](/Tilda-PlanPlace/plitnye-materialy/)** — он увеличивает расход материала в спецификации на заданный процент. Это основной способ заложить КИМ для типов п.м., м², м³ и р/хлыст (п.м.). Главное — не задвоить отход (не закладывать его и в коэффициент, и в цену/наценку).
...
- работает только для типов **п.м., м², м³ и р/хлыст (п.м.)**;
- для **р/шт.** не применяется (там запас уже даёт округление до целого листа).
